...
<div id="subject-name_form_con" class="client-val-form">
    <?php if ($model->isNewRecord) : ?>    <h3 id="create_header"><?php echo Yii::t('courses','Create New Subject');?></h3>
    <?php  elseif (!$model->isNewRecord):  ?>    <h3 id="update_header"><?php echo Yii::t('courses','Update Subject');?></h3>
    <?php   endif;  ?>
    <?php      $val_error_msg = Yii::t('courses','Error.SubjectName was not saved.');
    $val_success_message = ($model->isNewRecord) ?
            Yii::t('courses','Subject was created successfuly.') :
            Yii::t('courses','Subject was updated successfuly.');
  ?>
...
<?php   $formId='subject-name-form';
   ...
   ($model->isNewRecord)?CController::createUrl('defaultsubjects/ajax_create')
    ...
    <div style="font-weight:bold">'.Yii::t('courses','Please correct these errors:').'</div>
    ...
    <div class="row buttons" style="width:30%">
        <?php   echo CHtml::submitButton($model->isNewRecord ? Yii::t('courses','Save') : Yii::t('courses','Save'),array('class' =>
        'formbut')); ?>    </div>
