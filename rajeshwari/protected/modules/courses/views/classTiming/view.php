

<?php
$this->breadcrumbs=array(
	Yii::t('Timing','Class Timings')=>array('/courses'),
	$model->name,
);

/**$this->menu=array(
	array('label'=>'List ', 'url'=>array('index')),
	array('label'=>'Create ', 'url'=>array('create')),
	array('label'=>'Update ', 'url'=>array('update', 'id'=>$model->)),
	array('label'=>'Delete ', 'url'=>'#', 'linkOptions'=>array('submit'=>array('delete','id'=>$model->),'confirm'=>'Are you sure you want to delete this item?')),
	array('label'=>'Manage ', 'url'=>array('admin')),
);*/
?>

<h1><?php echo Yii::t('Timing','View Class Timings');?></h1>
<div class="tableinnerlist" style="padding-right:25px;">


<table width="100%" border="0" cellspacing="1" cellpadding="0">
  <tr>
    <td><?php echo Yii::t('Subjects','Name');?></td>
    <td><?php echo $model->name; ?></td>
  </tr>
  <tr>
    <td><?php echo Yii::t('Subjects','Start Time');?></td>
    <td><?php echo $model->start_time; ?></td>
  </tr>
  <tr>
    <td><?php echo Yii::t('Subjects','End Time');?></td>
    <td><?php echo $model->end_time; ?></td>
  </tr>
  <tr>
    <td><?php echo Yii::t('Subjects','Is Break');?></td>
    <td>
		<?php 
		if($model->is_break)
		{
			echo 'Yes';
		}
		else
		{
			echo 'No';	
		} 
		?>
	</td>
  </tr>
</table>

</div>