...
.flash-success{ color:#F30;
	font-size:12px;
	margin-bottom:10px;}
</style>
<?php
$this->breadcrumbs=array(
	Yii::t('weekdays','Weekdays'),
);
?>
...
//echo CHtml::dropDownList('mydropdownlist','mydropdownlist',$data,array('onchange'=>'getid();','id'=>'drop','options'=>array($_REQUEST['id']=>array('selected'=>true))));
 ...
 <?php $form=$this->beginWidget('CActiveForm', array('id'=>'courses-form','enableAjaxValidation'=>false,)); ?>
	  ...
	  ?> 
      <?php echo CHtml::submitButton($model->isNewRecord ? Yii::t('weekdays','Save') : Yii::t('weekdays','Save'),array('class'=>'formbut')); ?>          
   <?php $this->endWidget(); ?>              
</div>
</div>
</div>
...
<h3><?php echo Yii::t('weekdays','Week Days'); ?></h3>
<?php
    Yii::app()->clientScript->registerScript(
       'myHideEffect',
       '$(".flash-success").animate({opacity: 1.0}, 3000).fadeOut("slow");',
