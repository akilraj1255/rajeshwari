...
{
	$title = Yii::t('fees','Partial Fee Payment');
}
elseif(Yii::app()->controller->action->id=='Editfees')
{
	$title = Yii::t('fees','Edit Partial Fee Payment');
}
$this->beginWidget('zii.widgets.jui.CJuiDialog',array(
                'id'=>'partialfees'.$model->id,
                'options'=>array(
                    'title'=>Yii::t('fees',$title),
                    'autoOpen'=>true,
                    'modal'=>'true',
                    'width'=>'auto',
                    'height'=>'auto',
...
<div style="padding:10px 20px 10px 20px;">
        ...
        <div class="formConInner" style="width:auto">
        	<table>
            	<tr>
                    ...
                    <td class="partial_fee_heading">
                    	<?php echo Yii::t('fees','Admission No :'); ?>
                    </td>
                    <td>
                    	<?php
                ...
                <tr>
                	<td class="partial_fee_heading">
                    	<?php echo Yii::t('fees','Balance To Pay :'); ?>
                    </td>
                    <?php
    ...
    <div class="formCon">
        ...
        <?php $form=$this->beginWidget('CActiveForm', array(
            ...
            <div class="row">
                <?php echo $form->labelEx($model,'Amount Paid'); ?>
                <?php 
					if(Yii::app()->controller->action->id=='Partialfees')
					{
						echo $form->textField($model,'fees_paid',array('value'=>'','size'=>60,'maxlength'=>120)); 
					}
					elseif(Yii::app()->controller->action->id=='Editfees')
					{
						echo $form->textField($model,'fees_paid',array('size'=>60,'maxlength'=>120)); 
					}
				?>
                <?php echo $form->error($model,'fees_paid'); ?>
            </div>
            <br />
            <div class="row buttons">
				 ...
				 if(Yii::app()->controller->action->id=='Partialfees')
				 {
					 echo CHtml::ajaxSubmitButton(Yii::t('fees','Pay'),CHtml::normalizeUrl(array('FinanceFees/Partialfees')),array('dataType'=>'json','success'=>'js: 				
					 function(data) {
				 ...
				 elseif(Yii::app()->controller->action->id=='Editfees')
				 {
					echo CHtml::ajaxSubmitButton(Yii::t('fees','Save'),CHtml::normalizeUrl(array('FinanceFees/Editfees')),array('dataType'=>'json','success'=>'js: 				
					 function(data) {
									if(data.status == "success")
									{
										$("#partialfees'.$id.'").dialog("close");
