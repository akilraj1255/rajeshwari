...
<div id="finance-fee-collections_form_con" class="client-val-form">
    <?php if ($model->isNewRecord) : ?>    <h3 id="create_header"><?php echo Yii::t('fees','Create New Fee Collections');?></h3>
    <?php  elseif (!$model->isNewRecord and strtotime($model->start_date) > time()):  ?>    <h3 id="update_header"><?php echo Yii::t('fees','Update');?></h3>
    <?php  elseif (!$model->isNewRecord and strtotime($model->start_date) <= time()):  ?>    <h3 id="update_header"><?php echo Yii::t('fees','Notice');?></h3>
    <?php   endif;  ?>
    <?php      $val_error_msg = Yii::t('fees','Error.FinanceFeeCollections was not saved.');
    $val_success_message = ($model->isNewRecord) ?
            Yii::t('fees','Fee Collections was created successfuly.') :
            Yii::t('fees','Fee Collections  was updated successfuly.');
  ?>
...
<?php   $formId='finance-fee-collections-form';
   ...
   ($model->isNewRecord)?CController::createUrl('financeFeeCollections/ajax_create')
	...
	?>
            ...
            <?php echo $form->dropDownList($model,'fee_category_id',$data,array('empty' => 'Select Fee Category')); ?>
    ...
    <div class="row">
        ...
        <span style="margin-top:0px; position: relative; right:0px; top:-23px;" id="success-FinanceFeeCollections_name"
              class="hid input-notification-success  success png_bg right"></span>
        <div>
            <small></small>
        </div>
        ...
        <div class="row">
			...
			$this->widget('zii.widgets.jui.CJuiDatePicker', array(
			 ...
			 ?>
        <span style="margin-top:0px; position: relative; right:0px; top:-23px;" id="success-FinanceFeeCollections_start_date"
              class="hid input-notification-success  success png_bg right"></span>
        <div>
            <small></small>
        </div>
        ...
        <div class="row">
            ...
            <?php //echo $form->textField($model,'end_date');
			 ...
			 ?>
        <span style="margin-top:0px; position: relative; right:0px; top:-23px;" id="success-FinanceFeeCollections_end_date"
              class="hid input-notification-success  success png_bg right"></span>
        <div>
            <small></small>
        </div>
        ...
        <div class="row">
            ...
            <?php //echo $form->textField($model,'due_date');
			 ...
			 ?>
        <span style="margin-top:0px; position: relative; right:0px; top:-23px;" id="success-FinanceFeeCollections_due_date"
              class="hid input-notification-success  success png_bg right"></span>
        <div>
            <small></small>
        </div>
    ...
    <div class="row buttons" style="width:30%;">
        <?php   echo CHtml::submitButton($model->isNewRecord ? Yii::t('fees','Submit') : Yii::t('fees','Save'),array('class' =>
        'formbut')); ?>    </div>
...
?>
		<div class="yellow_bx" style="background-image:none;width:400px;padding-bottom:45px;margin-top:20px;margin-right:30px;color:#E26214;font-family:'Trebuchet MS', Arial, Helvetica, sans-serif;font-size: 14px;">
               ...
               <?php echo Yii::t('fees','<i>Cannot update because the fee collection has already begun.</i>').'<br/>'; ?>
               <?php echo Yii::t('fees','<i>You can either delete this collection or create a new collection.</i>'); ?> 
