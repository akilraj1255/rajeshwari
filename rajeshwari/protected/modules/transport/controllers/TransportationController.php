...
class TransportationController extends RController
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','autocomplete','routes','studentsearch','autocomplete1','settings','error','reallot','remove'),
				'users'=>array('*'),
			),
	...
	public function actionSave()
	{
		...
		$model->stop_id=$_REQUEST['transportation']['stop_id'];
		if($model->save())
			{
				$this->redirect(array('view','id'=>$model->id));
			}
	...
	public function actionReallot($id)
	{
		...
		if(isset($_POST['Transportation']))
		{
			...
			if(@$allot)
			{
				$allot->student_id = $_POST['Transportation']['student_id'];
				$allot->stop_id = $_POST['Transportation']['stop_id'];
				$allot->save();
				$this->redirect(array('view','id'=>$model->id));
			}
